...
class ContentPublishScheduledCommand extends Command
{
    ...
    public function handle(): int
    {
        ...
        try {
            ...
            $scheduledAnnouncements = Announcement::where('status', 'Scheduled')
                ->where('start_date', '<=', now())
                ->get();

            $publishedAnnouncementsCount = 0;
            foreach ($scheduledAnnouncements as $announcement) {
                if ($announcement->shouldPublish()) {
                    $announcement->publish();
                    $publishedAnnouncementsCount++;
                }
            }

            $this->info("✅ Published {$publishedAnnouncementsCount} scheduled announcements");
