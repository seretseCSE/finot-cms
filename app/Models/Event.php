...
class Event extends BaseModel
{
    ...
    protected static function boot(): void
    {
        ...
        static::creating(function ($event) {
            if (empty($event->created_by)) {
                $event->created_by = auth()->check() ? auth()->id() : 1;
            }
        });
    }
        ...
        while ($currentDate->lte($endDate)) {
            ...
            if ($currentDate->lte($endDate)) {
                Event::create([
                    ...
                    'created_by' => auth()->check() ? auth()->id() : ($this->created_by ?? 1), // Use authenticated user or fallback
                ]);
            }
        }
    }
    ...
    public function getFeaturedImageUrlAttribute(): ?string
    {
        ...
        // If it starts with 'events/', it's in storage
        if (str_starts_with($this->featured_image, 'events/')) {
