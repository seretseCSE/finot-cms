...
class InventoryItem extends BaseModel
{
    ...
    protected static function boot(): void
    {
        ...
        static::creating(function ($model) {
            ...
            if (! $model->item_code) {
                $model->item_code = $model->generateItemCode();
            }
        });
    }
    ...
    /**
     * Generate unique item code (I00001, I00002, etc.)
     */
    public function generateItemCode(): string
    {
        $lastItem = self::orderBy('id', 'desc')->first();
        $nextNumber = $lastItem ? ((int) substr($lastItem->item_code, 1) + 1) : 1;

        return 'I' . str_pad((string) $nextNumber, 5, '0', STR_PAD_LEFT);
    }
}
