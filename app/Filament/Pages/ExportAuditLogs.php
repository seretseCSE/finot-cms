...
class ExportAuditLogs extends Page
{
    ...
    {
        return \App\Support\RoleGate::can('page.system.audit-logs-export');
    }
    ...
    public function export(): \Symfony\Component\HttpFoundation\Response
    {
        try {
            ...
            if (! empty($data['entity_type']) && $data['entity_type'] !== 'all') {
            ...
            // Log the export action for GDPR/compliance traceability
    ...
    private function exportToPdf($query, string $filename): \Symfony\Component\HttpFoundation\Response
    {
        ...
        $content .= '<tr><th>Date</th><th>User</th><th>Action</th><th>Entity Type</th><th>Entity ID</th><th>IP Address</th></tr>';

        foreach ($records as $record) {
            $content .= '<tr>';
            $content .= '<td>'.$record->created_at->format('Y-m-d H:i:s').'</td>';
            $content .= '<td>'.($record->user?->name ?? 'System').'</td>';
            $content .= '<td>'.ucfirst($record->action_type ?? 'N/A').'</td>';
            $content .= '<td>'.($record->entity_type ? class_basename($record->entity_type) : 'N/A').'</td>';
            $content .= '<td>'.($record->entity_id ?? 'N/A').'</td>';
            $content .= '<td>'.($record->ip_address ?? 'N/A').'</td>';
            $content .= '</tr>';
        }
