                ...
                \Filament\Schemas\Components\Section::make('Donation Details')
                    ->schema([
                        ...
                        Forms\Components\Select::make('donation_type')
                            ...
                            ->required()
                            ->helperText('Select fund or purpose for this donation'),
                        ...
                        Forms\Components\Select::make('bank_account_id')
                            ->label('Bank Account')
                            ->options(fn () => \App\Models\BankAccount::active()->pluck('account_name', 'id'))
                            ->placeholder('Select bank account if applicable')
                            ->nullable()
                            ->helperText('Link donation to bank account for balance tracking'),
        ...
        return $table
            ->columns([
                ...
                Tables\Columns\TextColumn::make('bankAccount.account_name')
                    ->label('Bank Account')
                    ->sortable()
                    ->formatStateUsing(fn ($record) => $record->bankAccount?->account_name ?? 'No Account'),
            ...
            ->modifyQueryUsing(fn (Builder $query) => $query->with(['bankAccount']))
            ->filters([
