...
class ViewMemberTimelineOptimized extends Page implements HasForms
{
    use InteractsWithForms;

    protected static string $resource = MemberResource::class;

    protected string $view = 'filament.resources.member.pages.view-member-timeline';
    ...
    protected function loadEvents(bool $append): void
    {
        ...
        $result = Cache::remember($cacheKey, now()->addMinutes(5), function () use ($filters, $tab, $allowed) {
            $query = $this->buildOptimizedTimelineQuery($filters, $tab, $allowed);
            ...
            $hasMore = $events->count() > $this->perPage;
            $events = $events->take($this->perPage)->values();
    ...
    /**
     * Build optimized timeline query with single Eloquent query and eager loading
     */
    protected function buildOptimizedTimelineQuery(array $filters, string $tab, array $allowed): QueryBuilder
    {
        $memberId = $filters['member_id'] ?? null;
        $memberName = $filters['name'] ?? null;
        $groupName = $filters['group_name'] ?? null;
        $parentName = $filters['parent_name'] ?? null;

        $baseQuery = DB::table('members as m')
        ...
        // Apply member filters first
        if (filled($memberId)) {
            $baseQuery->where('m.id', (int) $memberId);
        }

        if (filled($memberName)) {
            $name = trim($memberName);
            $baseQuery->where(function ($q) use ($name): void {
                $q->where('m.first_name', 'like', "%{$name}%")
                    ->orWhere('m.father_name', 'like', "%{$name}%");
            });
        }

        if (filled($groupName) && DbSchema::hasTable('member_group_assignments')) {
            $baseQuery->whereExists(function ($sub) use ($groupName): void {
                $sub->select(DB::raw(1))
                    ->from('member_group_assignments as mga2')
                    ->join('member_groups as g2', 'g2.id', '=', 'mga2.group_id')
                    ->join('members as m2', 'm2.id', '=', 'mga2.member_id')
                    ->whereColumn('mga2.member_id', 'm.id')
                    ->whereNull('mga2.effective_to')
                    ->where('g2.name', 'like', "%{$groupName}%");
            });
        }

        if (filled($parentName) && DbSchema::hasTable('member_parent_guardians')) {
            $baseQuery->whereExists(function ($sub) use ($parentName): void {
                $sub->select(DB::raw(1))
                    ->from('member_parent_guardians as mpg2')
                    ->join('members as m2', 'm2.id', '=', 'mpg2.member_id')
                    ->whereColumn('mpg2.member_id', 'm.id')
                    ->where('mpg2.parent_name', 'like', "%{$parentName}%");
            });
        }

        // Build union queries with proper eager loading
        $queries = [];
        ...
        if ($include('groups') && DbSchema::hasTable('member_group_assignments')) {
            ...
            // Apply member filters to group queries
            if (filled($memberId)) {
                $groupQuery->where('m.id', (int) $memberId);
            }
            if (filled($memberName)) {
                $groupQuery->where(function ($q) use ($memberName): void {
                    $q->where('m.first_name', 'like', "%{$memberName}%")
                        ->orWhere('m.father_name', 'like', "%{$memberName}%");
                });
            }
            if (filled($groupName)) {
                $groupQuery->whereExists(function ($sub) use ($groupName): void {
                    $sub->select(DB::raw(1))
                        ->from('member_group_assignments as mga2')
                        ->join('member_groups as g2', 'g2.id', '=', 'mga2.group_id')
                        ->join('members as m2', 'm2.id', '=', 'mga2.member_id')
        ...
        if ($include('education')) {
            if (DbSchema::hasTable('student_enrollments')) {
                $educationQuery = DB::table('student_enrollments as se')
                    ...
                    ->join('school_classes as sc', 'sc.id', '=', 'se.class_id')
        ...
        if (empty($queries)) {
            return $baseQuery->fromSub(DB::query()->select([
        ...
        return $baseQuery->fromSub($union, 'timeline');
    }
