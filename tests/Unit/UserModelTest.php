...
class UserModelTest extends TestCase
{
    ...
    public function test_user_cannot_reuse_password_in_history(): void
    {
        ...
        $firstPassword = 'FirstPass123';
        $secondPassword = 'SecondPass456';

        // Update to first password - old factory password goes to history
        $user->updatePassword($firstPassword);

        // Update to second password - first password goes to history
        $user->updatePassword($secondPassword);

        // The first password should now be in history
        $this->assertTrue($user->isPasswordInHistory($firstPassword));
    }
    ...
    public function test_user_gets_lockout_message(): void
    {
        ...
        $lockedUser = User::factory()->create([
            'is_locked' => true,
            'locked_until' => $frozen->copy()->addMinutes(5),
        ]);
    ...
    {
        $frozen = \Carbon\Carbon::parse('2024-01-15 12:00:00');
        \Carbon\Carbon::setTestNow($frozen);

        $lockedUser = User::factory()->create([
            'is_locked' => true,
            'locked_until' => $frozen->copy()->addMinutes(10),
        ]);
