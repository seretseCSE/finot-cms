...
class SessionLimitTest extends TestCase
{
    ...
    public function test_session_limit_max_three_devices(): void
    {
        ...
        $activeSessions = UserSession::where('user_id', $user->id)
            ->where('last_activity', '>=', now()->subMinutes(config('finot.session_timeout_minutes', 30)))
            ->count();
    ...
    public function test_inactive_sessions_not_counted(): void
    {
        ...
        UserSession::create([
            ...
            'device_info' => json_encode(['name' => 'Expired Device']),
    ...
    public function test_session_can_be_revoked(): void
    {
        ...
        // Revoke the session by expiring it
        $session->update(['last_activity' => now()->subHours(2)]);
    ...
    public function test_revoking_session_allows_new_session(): void
    {
        ...
        // Revoke one session by expiring it
        $sessionToRevoke = UserSession::where('user_id', $user->id)->first();
        $sessionToRevoke->update(['last_activity' => now()->subHours(2)]);
    ...
    public function test_session_includes_device_info(): void
    {
        ...
        $session = UserSession::create([
            'user_id' => $user->id,
            'session_token' => Str::random(40),
            'device_info' => json_encode(['name' => 'Test Device']),
            'ip_address' => '127.0.0.1',
            'last_activity' => now(),
        ]);

        $deviceInfo = json_decode($session->device_info, true);
        $this->assertEquals('Test Device', $deviceInfo['name']);
    ...
    public function test_oldest_session_revoked_when_limit_exceeded(): void
    {
        ...
        // Create 3 sessions with different timestamps (all within active window)
        $session1 = $this->createSessionForUser($user, now()->subMinutes(25));
        $session2 = $this->createSessionForUser($user, now()->subMinutes(20));
        $session3 = $this->createSessionForUser($user, now()->subMinutes(15));

        // Try to add a 4th - should revoke oldest
        $this->handleSessionLimitExceeded($user);

        // The oldest session should be revoked
        $this->assertFalse($session1->fresh()->isActive());

        // Others should still be active
        $this->assertTrue($session2->fresh()->isActive());
        $this->assertTrue($session3->fresh()->isActive());
    }
    ...
    public function test_active_session_detection(): void
    {
        ...
        $activeSession = UserSession::create([
            'user_id' => $user->id,
            'session_token' => Str::random(40),
            'device_info' => json_encode(['name' => 'Active Device']),
            'ip_address' => '127.0.0.1',
            'last_activity' => now(),
        ]);
        ...
        $inactiveSession = UserSession::create([
            'user_id' => $user->id,
            'session_token' => Str::random(40),
            'device_info' => json_encode(['name' => 'Inactive Device']),
            'ip_address' => '127.0.0.1',
     ...
     */
    protected function createSessionsForUser(User $user, int $count): void
    {
        for ($i = 0; $i < $count; $i++) {
    ...
    protected function createSessionForUser(User $user, $lastActivity = null, string $device = 'Test Device', string $ip = '127.0.0.1'): UserSession
    {
        return UserSession::create([
            'user_id' => $user->id,
            'session_token' => Str::random(40),
            'device_info' => json_encode(['name' => $device]),
            'ip_address' => $ip,
    ...
    protected function handleSessionLimitExceeded(User $user): void
    {
        ...
        if ($activeCount >= $maxSessions) {
            ...
            if ($oldestSession) {
                $oldestSession->update(['last_activity' => now()->subHours(2)]);
            }
        }
    }
    ...
    {
        return $session->isActive() && !$this->isSessionExpired($session);
    }
}
