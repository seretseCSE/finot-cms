...
export default function serviceWorkerPlugin(options = {}) {
    return {
        ...
        generateBundle(options, bundle) {
            ...
            Object.keys(bundle).forEach(fileName => {
                if (fileName.endsWith('.js') || fileName.endsWith('.css')) {
                    buildInfo.assets.push('/' + fileName);
                }
            });

            // Generate service worker with build info
            const serviceWorkerContent = generateServiceWorker(buildInfo);
            ...
            console.log('✅ Service worker generated with cache version:', buildInfo.hash);
        }
    };
}

function generateServiceWorker(buildInfo) {
    const cacheName = `finot-cache-${buildInfo.hash}`;
    const apiCacheName = `finot-api-cache-${buildInfo.hash}`;

    return `
const BUILD_INFO = ${JSON.stringify(buildInfo)};
const CACHE_NAME = '${cacheName}';
const API_CACHE_NAME = '${apiCacheName}';

// Dynamic assets from build
const DYNAMIC_ASSETS = ${JSON.stringify(buildInfo.assets)};
