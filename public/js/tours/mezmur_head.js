(() => {
    const steps = [
        { target: '[data-tour-target="dashboard"]', title: 'Mezmur (Choir) Overview', content: 'Overview of choir and music activities.', side: 'bottom' },
        { target: '[data-tour-target="choir-members"]', title: 'Choir Members', content: 'Manage choir members and roles.', side: 'right' },
        { target: '[data-tour-target="music-library"]', title: 'Music Library', content: 'Manage songs, lyrics, and resources.', side: 'left' },
        { target: '[data-tour-target="rehearsals"]', title: 'Rehearsals', content: 'Schedule rehearsals and track participation.', side: 'bottom' },
    ];
    ...
    function endTour() {
        ...
        localStorage.removeItem('mezmur_head_tour_completed');
    }
    ...
    function startTour() {
        if (localStorage.getItem('mezmur_head_tour_completed')) return;
        createTourOverlay();
        showStep(0);
    }

    if (window.location.hash === '#tour' || !localStorage.getItem('mezmur_head_tour_visited')) {
        startTour();
        localStorage.setItem('mezmur_head_tour_visited', '1');
    }

    window.startMezmurHeadTour = startTour;
    window.endMezmurHeadTour = endTour;
})();
