(() => {
    const steps = [
        { target: '[data-tour-target="dashboard"]', title: 'Admin Dashboard', content: 'System overview and quick access widgets.', side: 'bottom' },
        { target: '[data-tour-target="users"]', title: 'Users', content: 'Create users and assign roles.', side: 'right' },
        { target: '[data-tour-target="reports"]', title: 'Reports', content: 'Generate system reports and exports.', side: 'left' },
        { target: '[data-tour-target="settings"]', title: 'Settings', content: 'Configure common system settings.', side: 'bottom' },
    ];
    ...
    function startTour() {
        if (localStorage.getItem('admin_tour_completed')) return;
        createTourOverlay();
        showStep(0);
    }

    if (window.location.hash === '#tour' || !localStorage.getItem('admin_tour_visited')) {
        startTour();
        localStorage.setItem('admin_tour_visited', '1');
    }

    window.startAdminTour = startTour;
    window.endAdminTour = endTour;
})();
