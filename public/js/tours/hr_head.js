(() => {
    const steps = [
        {
            ...
            title: 'HR Dashboard',
            content: 'Overview of member and staffing metrics.',
            side: 'bottom',
        },
        {
            target: '[data-tour-target="members"]',
            title: 'Members',
            content: 'Create and manage member records here.',
            side: 'right',
        },
        {
            target: '[data-tour-target="roles-permissions"]',
            title: 'Roles & Permissions',
            content: 'Assign roles and manage access control.',
            side: 'left',
        },
        {
            target: '[data-tour-target="reports"]',
            title: 'Reports',
            content: 'Generate HR/member related reports.',
            side: 'bottom',
        },
    ];
    ...
    function endTour() {
        ...
        localStorage.removeItem('hr_head_tour_completed');
    }
    ...
    function startTour() {
        if (localStorage.getItem('hr_head_tour_completed')) return;
        createTourOverlay();
        showStep(0);
    }

    if (window.location.hash === '#tour' || !localStorage.getItem('hr_head_tour_visited')) {
        startTour();
        localStorage.setItem('hr_head_tour_visited', '1');
    }

    window.startHrHeadTour = startTour;
    window.endHrHeadTour = endTour;
})();
