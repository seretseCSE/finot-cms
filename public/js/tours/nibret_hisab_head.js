(() => {
    const steps = [
        { target: '[data-tour-target="dashboard"]', title: 'Assets & Accounts', content: 'Overview of assets and accounting tasks.', side: 'bottom' },
        { target: '[data-tour-target="inventory"]', title: 'Inventory', content: 'Manage church assets and inventory items.', side: 'right' },
        { target: '[data-tour-target="asset-tracking"]', title: 'Asset Tracking', content: 'Track asset assignment and lifecycle.', side: 'left' },
        { target: '[data-tour-target="reports"]', title: 'Reports', content: 'Generate inventory and asset reports.', side: 'bottom' },
    ];
    ...
    function endTour() {
        ...
        localStorage.removeItem('nibret_hisab_head_tour_completed');
    }
    ...
    function startTour() {
        if (localStorage.getItem('nibret_hisab_head_tour_completed')) return;
        createTourOverlay();
        showStep(0);
    }

    if (window.location.hash === '#tour' || !localStorage.getItem('nibret_hisab_head_tour_visited')) {
        startTour();
        localStorage.setItem('nibret_hisab_head_tour_visited', '1');
    }

    window.startNibretHisabHeadTour = startTour;
    window.endNibretHisabHeadTour = endTour;
})();
