(() => {
    const steps = [
        { target: '[data-tour-target="dashboard"]', title: 'AV Dashboard', content: 'Overview of media and service support tasks.', side: 'bottom' },
        { target: '[data-tour-target="media-library"]', title: 'Media Library', content: 'Upload and organize photos/videos/audio.', side: 'right' },
        { target: '[data-tour-target="live-stream"]', title: 'Live Stream', content: 'Configure livestream and service playback.', side: 'left' },
        { target: '[data-tour-target="reports"]', title: 'Reports', content: 'Review media usage and activity.', side: 'bottom' },
    ];
    ...
    function endTour() {
        ...
        localStorage.removeItem('av_head_tour_completed');
    }
    ...
    function startTour() {
        if (localStorage.getItem('av_head_tour_completed')) return;
        createTourOverlay();
        showStep(0);
    }

    if (window.location.hash === '#tour' || !localStorage.getItem('av_head_tour_visited')) {
        startTour();
        localStorage.setItem('av_head_tour_visited', '1');
    }

    window.startAvHeadTour = startTour;
    window.endAvHeadTour = endTour;
})();
