(() => {
    ...
    function endTour() {
        ...
        localStorage.removeItem('staff_tour_completed');
    }
    ...
    function startTour() {
        if (localStorage.getItem('staff_tour_completed')) return;
        createTourOverlay();
        showStep(0);
    }

    if (window.location.hash === '#tour' || !localStorage.getItem('staff_tour_visited')) {
        startTour();
        localStorage.setItem('staff_tour_visited', '1');
    }

    window.startStaffTour = startTour;
    window.endStaffTour = endTour;
})();
