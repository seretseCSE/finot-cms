(() => {
    const steps = [
        { target: '[data-tour-target="dashboard"]', title: 'Department Secretary', content: 'Manage documents and meeting records.', side: 'bottom' },
        { target: '[data-tour-target="documents"]', title: 'Documents', content: 'Upload and organize department documents.', side: 'right' },
        { target: '[data-tour-target="meeting-minutes"]', title: 'Meeting Minutes', content: 'Record and manage meeting minutes.', side: 'left' },
        { target: '[data-tour-target="calendar"]', title: 'Calendar', content: 'Track upcoming meetings and tasks.', side: 'bottom' },
    ];
    ...
    function endTour() {
        ...
        localStorage.removeItem('department_secretary_tour_completed');
    }
    ...
    function startTour() {
        if (localStorage.getItem('department_secretary_tour_completed')) return;
        createTourOverlay();
        showStep(0);
    }

    if (window.location.hash === '#tour' || !localStorage.getItem('department_secretary_tour_visited')) {
        startTour();
        localStorage.setItem('department_secretary_tour_visited', '1');
    }

    window.startDepartmentSecretaryTour = startTour;
    window.endDepartmentSecretaryTour = endTour;
})();
