(() => {
    const steps = [
        { target: '[data-tour-target="dashboard"]', title: 'Inventory Dashboard', content: 'Overview of assets, items, and stock movements.', side: 'bottom' },
        { target: '[data-tour-target="inventory-items"]', title: 'Items', content: 'Add/update inventory items and details.', side: 'right' },
        { target: '[data-tour-target="stock-movements"]', title: 'Stock Movements', content: 'Track incoming/outgoing stock movement records.', side: 'left' },
        { target: '[data-tour-target="reports"]', title: 'Reports', content: 'Export inventory reports when needed.', side: 'bottom' },
    ];
    ...
    function endTour() {
        ...
        localStorage.removeItem('inventory_staff_tour_completed');
    }
    ...
    function startTour() {
        if (localStorage.getItem('inventory_staff_tour_completed')) return;
        createTourOverlay();
        showStep(0);
    }

    if (window.location.hash === '#tour' || !localStorage.getItem('inventory_staff_tour_visited')) {
        startTour();
        localStorage.setItem('inventory_staff_tour_visited', '1');
    }

    window.startInventoryStaffTour = startTour;
    window.endInventoryStaffTour = endTour;
})();
