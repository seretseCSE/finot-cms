(() => {
    const steps = [
        { target: '[data-tour-target="dashboard"]', title: 'Finance Dashboard', content: 'Overview of contributions, donations, and summaries.', side: 'bottom' },
        { target: '[data-tour-target="contributions"]', title: 'Contributions', content: 'Record, review, and reconcile contributions.', side: 'right' },
        { target: '[data-tour-target="donations"]', title: 'Donations', content: 'Track donations and donor details.', side: 'left' },
        { target: '[data-tour-target="exports"]', title: 'Exports', content: 'Export finance reports. Exports are audit-logged.', side: 'bottom' },
    ];
    ...
    function endTour() {
        ...
        localStorage.removeItem('finance_head_tour_completed');
    }
    ...
    function startTour() {
        if (localStorage.getItem('finance_head_tour_completed')) return;
        createTourOverlay();
        showStep(0);
    }

    if (window.location.hash === '#tour' || !localStorage.getItem('finance_head_tour_visited')) {
        startTour();
        localStorage.setItem('finance_head_tour_visited', '1');
    }

    window.startFinanceHeadTour = startTour;
    window.endFinanceHeadTour = endTour;
})();
