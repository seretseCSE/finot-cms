(() => {
    const steps = [
        { target: '[data-tour-target="dashboard"]', title: 'Charity Dashboard', content: 'Overview of aid programs and beneficiary activity.', side: 'bottom' },
        { target: '[data-tour-target="beneficiaries"]', title: 'Beneficiaries', content: 'Register and manage beneficiaries here.', side: 'right' },
        { target: '[data-tour-target="aid-programs"]', title: 'Aid Programs', content: 'Create and track aid programs and distributions.', side: 'left' },
        { target: '[data-tour-target="reports"]', title: 'Reports', content: 'Generate charity and distribution reports.', side: 'bottom' },
    ];
    ...
    function endTour() {
        ...
        localStorage.removeItem('charity_head_tour_completed');
    }
    ...
    function startTour() {
        if (localStorage.getItem('charity_head_tour_completed')) return;
        createTourOverlay();
        showStep(0);
    }

    if (window.location.hash === '#tour' || !localStorage.getItem('charity_head_tour_visited')) {
        startTour();
        localStorage.setItem('charity_head_tour_visited', '1');
    }

    window.startCharityHeadTour = startTour;
    window.endCharityHeadTour = endTour;
})();
