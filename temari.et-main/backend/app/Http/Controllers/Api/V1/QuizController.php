        ...
        if ($request->boolean('platform')) {
            ...
            $query
                ->when($request->filled('grade_level_id'), fn ($q) => $q->whereHas(
                    'targetAssignments.section',
                    fn ($s) => $s->where('grade_level_id', $request->integer('grade_level_id')),
                ))
                ->when($request->filled('subject_id'), fn ($q) => $q->whereHas(
                    'targetAssignments',
                    fn ($t) => $t->where('subject_id', $request->integer('subject_id')),
                ))
                ->when($request->filled('section_id'), fn ($q) => $q->whereHas(
                    'targetAssignments',
                    fn ($t) => $t->where('section_id', $request->integer('section_id')),
                ));
        }
