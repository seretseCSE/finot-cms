...
class CourseMaterialController extends Controller
{
    public function index(Request $request): JsonResponse
    {
        ...
        $materials = $query
            ...
            ->when($request->filled('section_id'), fn ($q) => $q->whereHas(
                'targets.subjectAssignment.section', fn ($s) => $s->where('sections.id', $request->integer('section_id')),
            ))
    ...
    private function applyGradeFilter($query, int $gradeLevelId): void
    {
        ...
        $query->where(fn ($q) => $q
            ...
            ->when($gradeSort !== null, fn ($q2) => $q2->orWhere(fn ($w) => $w
